...
%where to save data
save_string=strcat("simulations2/data_T100_dt-3_numnode"+num2str(num_nodes)+...
...
for ia=1:size(couplings, 2)
    ...
    for is=1:size(pre_s,2)
            sigma=pre_s(is); 
            disp([is, sigma]);
            [x_all, x_tau_all, dt_x_all,dt_x_all_v2, adjacency, betas]  = simulate_cubicdecay_tseries(sigma,num_simulations,alpha,num_nodes,...
...
%smaller dt
delta_t=0.00001;  %fine simulation tscale
res=0.01;  %coarse reconstruction t-scale
initial="zero"; %specify initial conditions
T=100;  %how many timesteps are evaluated
couplings=[1];  %coupling strength 
...
%where to save data
save_string=strcat("simulations2/data_T100_dt-5_numnode"+num2str(num_nodes)+...
    "n_incoming"+num2str(n_incoming)+ "sigma-5+3");
