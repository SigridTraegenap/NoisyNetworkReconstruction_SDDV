...
%reconstruct connectivities with nextstep algorithm
%linear decay data
        ...
        for it=1:size(times, 2)            
            ...
            if sum(isnan(dt_x_all(:)))>0
               res_auc(it, is)=NaN;
                ...
                %add noise with std 0.001 instead of rounding to three
                %digits
                [AUCs]=reoncstruction_nextstep(x_sample+randn(size(x_sample))*0.001, ...
                    dt_x_sample+randn(size(dt_x_sample))*0.001,...
                    adjacency,num_nodes);
...
save(strcat("simulations1/summmary_data_linear_T100_dt-4_numnode"+num2str(num_nodes)+...
    "n_incoming"+num2str(n_incoming)+ "sigma-5+3_nextstep_addnoise"), "res_auc")
...
key=strcat("linear_addnoise_nextstep_paramscan_sigma-5+3_alpha"+num2str(alpha));
print(key, "-dpdf")