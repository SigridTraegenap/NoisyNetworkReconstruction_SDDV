        ...
        for it=1:size(times, 2)            
            ...
            if sum(isnan(dt_x_all(:)))>0
               res_auc(it, is)=NaN;
                ...
                [AUCs]=reoncstruction_nextstep(round(x_sample,3), ...
...
save(strcat("simulations1/summmary_data_T100_dt-4_numnode"+num2str(num_nodes)+...
    "n_incoming"+num2str(n_incoming)+ "sigma-5+3_nextstep"), "res_auc");
...
key=strcat("nextstep_paramscan_sigma-5+3_alpha"+num2str(alpha));
print(key, "-dpdf")