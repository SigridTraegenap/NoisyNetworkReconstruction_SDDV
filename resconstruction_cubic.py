...
def simulate(N,M,sigma,T,c):
    ...
    for t in range(T):
        ...
        for m in range(L2-1):
            ...
            K1=dt*model(m,y[:,m],J,b,c,sigma)+sigma*(Wm-sqrtdt*Sm)
            K2=dt*model(m,y[:,m]+K1.reshape(N),J,b,c,sigma)+sigma*(Wm+sqrtdt*Sm)
...
def model(t,y,J,b,c,sigma):
    ...
    out=np.sum(J,axis=1)
    dydt = (- sigma**4 - out*y + np.dot(J,y)).reshape(N,1)
